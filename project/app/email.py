...
def send_password_reset_email(user):
    ...
    send_email('[secure Upload] Reset Your Password',
               ...
               text_body=render_template(
                   'email/reset_password.txt', user=user, token=tk),
               html_body=render_template(
                   'email/reset_password.html', user=user, token=tk)
               )
...
def send_confirmation_email(user):
    ...
    send_email('[secure Upload] Confirm your account',
               ...
               recipients=[user.email],
               text_body=render_template(
                   'email/confirm.txt', user=user, token=tk),
               html_body=render_template(
                   'email/confirm.html', user=user, token=tk)
               )
